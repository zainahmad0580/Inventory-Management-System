...
{
    public partial class Beautyitems : Form
    {
        public Beautyitems()
        {
            InitializeComponent();
        ...
        {
            DataClasses6DataContext dataClassesDataContext = new DataClasses6DataContext();

            List<Homeitem> st = new List<Homeitem>();
            var sf = (from s in dataClassesDataContext.Beauties select s).ToList();
            ...
            for (int i = 0; i < sf.Count; i += 3)
            {
                ...
                if (j < sf.Count)
                {
                    ...
                    if (j < sf.Count)
                    {
                        ...
                        if (ht[j] != null)
                            its[i].itr(ht[j].itemid, ht[j].imagepath, ht[j].descriptions, ht[j].price.ToString(), "Beauty");
                    }
