...
namespace WindowsFormsApp1
{
    public partial class women : Form
    {
        ...
        public void loaddata()
        {
            ...
            var sf = (from s in dataClasses1DataContext.Femaleitems select s).ToList();
            ...
            for (int i = 0; i < sf.Count; i += 3)
            {
                ...
                if (j < sf.Count)
                {
                    ...
                    if (j < sf.Count)
                    {
                        ...
                        if (ht[j] != null)
                            its[i].itr(ht[j].itemid, ht[j].imagepath, ht[j].descriptions, ht[j].price.ToString(), "Femaleitem");
                    }
